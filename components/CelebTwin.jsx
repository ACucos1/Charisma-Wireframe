      ...
      <div className={styles.twitterInnerProfileContainer}>
        ...
        <div className={styles.lowerHalfBg}>
          <div className={styles.twitterDetails} style={{overflow: 'hidden'}}>
            <div>
              <span className={styles.name}>{twitterObj && twitterObj.info.ensName}</span>
              {twitterObj && twitterObj.info.twitterIsVerified && <img src="/images/verified.png" alt="verfied-tick" />}
            </div>
            <span className={styles.username}>{twitterObj && twitterObj.info.twitterHandle}</span>
            <span style={{fontSize: '12px', fontWeight: 300}}>{twitterObj ? `${parseInt(twitterObj.info.twitterFollowersCount).toLocaleString('en-US')} followers` : 'Looks like you\' a real OG!'} </span>
            <p>{twitterObj ? twitterObj.info.twitterBio : 'We couldn\'t find a celeb with a similar result and wallet birthday to yours, congrats!'}</p>
          </div>
        </div>
        <img
          className={styles.avatar}
          src={twitterObj ? twitterObj.info.twitterProfileUrl : './images/personplaceholder.png'}
          alt="avatar"
        />
      </div>
    </div>
